...
#define TILESET_AMOUNT 6

extern u16 vramIndex;

static bool vramIndexLoaded;
static u16 vramTileSet[TILESET_AMOUNT];
...
void LEVEL_init(void) {
    ...
    VDP_loadTileSet(bgmap.tileset, vramTileSet[BACKGROUND], DMA);
...
static void LEVEL_draw(void) {
    ...
    VDP_setTileMapEx(BG_B, bgmap.tilemap, TILE_ATTR_FULL(PAL0, 0, 0, 0, vramTileSet[BACKGROUND]), 
                    0, 0, 0, 0, 32, 28, DMA);

    /* Draws ground. */
    for (u16 x = 0; x < 32; x += 2) {
        VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 0, 0, 0, vramTileSet[GROUND_GRASS]), 
                               x, 20, 2, 2);
    }

    for (u16 y = 0; y < 6; y += 2) {
        for (u16 x = 0; x < 32; x += 2) {
            VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 0, 0, 0, vramTileSet[GROUND]), 
                                   x, y + 22, 2, 2);
        }
    }

    /* Draws trees and grass. */
    VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 1, 0, 0, vramTileSet[GRASS]), 15, 18, 2, 2);
    VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 1, 0, 0, vramTileSet[GRASS]), 32, 18, 2, 2);

    VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 0, 0, 0, vramTileSet[TREE]), 20, 4, 8, 16);
    VDP_fillTileMapRectInc(BG_A, TILE_ATTR_FULL(PAL1, 0, 0, 0, vramTileSet[TREE_SMALL]), 4, 12, 8, 8);
}

static void initVramIndex(void) {
    vramTileSet[BACKGROUND] = vramIndex;
