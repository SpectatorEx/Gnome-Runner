...
void GS_tileScrolling(void) {
    ...
    for (u16 i = 2; i < 28; i++) {
        offsetTileA[i] = (offsetTileA[i] - SCROLL_SPEED) % 256;
    }
    ...
    for (u16 i = 0; i < 20; i++) {
        if (i < 6) {
            offsetTileBF[i] = fix16Add(offsetTileBF[i], FIX16(-0.5));
        } else if (i < 15) {
            offsetTileBF[i] = fix16Add(offsetTileBF[i], FIX16(-0.3));
        } else if (i < 20) {
            offsetTileBF[i] = fix16Add(offsetTileBF[i], FIX16(-1));
        }
